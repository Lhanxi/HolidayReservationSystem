 ...
 */
@Stateless
public class HotelInventory implements HotelInventoryRemote, HotelInventoryLocal {
    ...
    public boolean availableRoomsForReservation(Date startDate, Date endDate, RoomType roomType) {
        ...
        for (Reservation r : reservations) {
            if ((r.getStartDate()>= startDate && r.getStartDate() <= endDate) || (r.getEndDate() <= endDate && r.getEndDate() >= startDate)) {
                count += 1;
            }
        }
